...
import { encryptData, decryptData } from "../lib/message-crypto";
...
conversations.post("/", requireAuth, async (c) => {
    ...
    if (canvasId) {
        const canvas = await prisma.canvas.findFirst({ where: { id: canvasId, ownerId: user.id } });
        if (!canvas) return c.json({ error: "invalid canvas" }, 400);
    }
    const conv = await prisma.conversation.create({
        data: { userId: user.id, canvasId: canvasId ?? null, title: title?.slice(0, 200) || "Untitled" },
...
conversations.get("/:id", requireAuth, async (c) => {
    ...
    const messages = await prisma.message.findMany({
        ...
        select: { id: true, nodeId: true, role: true, contentEnc: true, iv: true, createdAt: true },
    });

    const out = messages.map(m => {
        let text = "";
        if (m.contentEnc && m.iv) {
            try { text = decryptData(m.contentEnc, m.iv); } catch { text = ""; }
        }
        return { id: m.id, nodeId: m.nodeId, role: m.role, content: text, createdAt: m.createdAt };
    });
...
conversations.post("/:id/messages", requireAuth, async (c) => {
    const user = c.get("user")!;
    const id = c.req.param("id");
    const body = await c.req.json().catch(() => null) as { nodeId?: string; role?: 'user' | 'assistant' | 'system'; content?: string } | null;
    if (!body?.nodeId || !body?.role || typeof body.content !== "string") {
        return c.json({ error: "nodeId, role, content required" }, 400);
    }

    // Verify conversation ownership
    const conv = await prisma.conversation.findFirst({ where: { id, userId: user.id, deletedAt: null } });
    if (!conv) return c.body(null, 404);
    ...
    const node = await prisma.node.findFirst({
        where: conv.canvasId
            ? { id: body.nodeId, canvasId: conv.canvasId, canvas: { ownerId: user.id } }
            : { id: body.nodeId, canvas: { ownerId: user.id } },
        select: { id: true },
    });
    if (!node) return c.json({ error: "invalid node" }, 400);

    const { encrypted, iv } = encryptData(body.content);
    ...
    return c.json({ id: msg.id, nodeId: msg.nodeId, role: msg.role, content: body.content, createdAt: msg.createdAt }, 201);
});
...
conversations.delete("/:id/messages/:messageId", requireAuth, async (c) => {
    ...
    const msg = await prisma.message.findFirst({
        where: { id: messageId, conversationId: id, node: { canvas: { ownerId: user.id } } },
        select: { id: true },
    });
