            ...
            >
                ...
                <div className="chat-header flex items-center justify-between px-4 py-3 bg-slate-800 text-white font-bold rounded-t-xl cursor-grab active:cursor-grabbing">
                    ...
                    <div className="controls flex items-center gap-x-2">
                        <button onClick={() => onScale(0.1)} title="Zoom In" className="flex items-center justify-center w-6 h-6 bg-slate-700 hover:bg-slate-600 rounded transition-colors">+</button>
                        <button onClick={() => onScale(-0.1)} title="Zoom Out" className="flex items-center justify-center w-6 h-6 bg-slate-700 hover:bg-slate-600 rounded transition-colors text-xl pb-1">–</button>
                        <button onClick={() => setMinimized(m => !m)} title={minimized ? "Maximize" : "Minimize"} className="flex items-center justify-center w-6 h-6 bg-slate-700 hover:bg-slate-600 rounded transition-colors">
                            {minimized ? '❐' : '—'}
                        </button>
                    </div>
                </div>
                ...
                {!minimized && (
                    <>
                        ...
                        <div className="flex-grow p-4 overflow-y-auto bg-slate-50 space-y-3">
                            {messages.map((msg, idx) => (
                                <div key={idx} className={`flex ${msg.sender === 'user' ? 'justify-end' : 'justify-start'}`}>
                                    <div className={`max-w-[80%] px-3 py-2 rounded-lg ${msg.sender === 'user' ? 'bg-blue-600 text-white' : 'bg-slate-200 text-slate-800'}`}>
                                        {msg.text}
                                    </div>
                                </div>
                            ))}
                        ...
                        {/* --- Input Area --- */}
                        <form onSubmit={handleSend} className="flex items-center gap-x-2 p-3 border-t border-slate-200 bg-white rounded-b-xl">
                            <input
                                type="text"
                                value={input}
